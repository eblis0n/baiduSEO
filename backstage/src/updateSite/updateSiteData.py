...
from src.updateSite.googleOnline.google_online_excel_public import googleOnlinePublic
from src.common.method.setAI import setAI
...
class updateSiteData():
    ...
    def main(self, sheet, groupName, witch, AIbase, db_config_slave, db_config_master):
        # 获取表格的所有行
                ...
                for i in range(1, len(row)):
                    ...
                    if row[i] != "" and row[i] != "0" and row[i] != 0:
                        if i == 1:
                            ...
                            videoDatas = self.sql.fetch_fixed_data_from_master(db_config_slave, int(row[i]))

                            commentInsert = []
                            for video in videoDatas:
                                print(f'在{video["vod_id"]},{video["vod_name"]} 中发表1条评论')
                                prompt = f"你是一个视频 迷，请以 第一人称角度 对 《{video['vod_name']}》 这视频 写 100字左右影评；要求：1、内容必须为正面，不能出现粗言秽语；"
                                data = {
                            ...
                            self.sql.batch_insert_gbook_sql(db_config_slave, gbookInsert, database_name)
                        # elif i == 4:
                        #
                        #     print(f"随机栏目 发布文章数量")
                        #
...
if __name__ == '__main__':
    ...
    baiduAIbase = {
        "API_KEY": "xdQn7jzugSzX86ubxfddcogY",
        "SECRET_KEY": "nsFnekuH55Pm640DLVsDrH48DF551AhS"
    }
    ...
    sheetTab = "影视站"
    groupName = ["107.148.0.119站群"]

    sheet = usite.run_vertical(target_url, sheetTab)
    witch = "baidu"
    if witch == "baidu":
        row = usite.main(sheet, groupName, witch, baiduAIbase, db_config_slave, db_config_master)
    else:
        row = usite.main(sheet, groupName, witch, AIbase, db_config_slave, db_config_master)
